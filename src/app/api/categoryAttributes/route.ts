  ...
  try {
    ...
    if (categoryId && attributeId) {
      const categoryAttribute =
        await CategoryAttributesService.getCategoryAttribute(
          categoryId,
          attributeId
        );
    ...
    if (categoryId) {
      const categoryAttributes =
        await CategoryAttributesService.getAttributesByCategoryId(categoryId);
      return NextResponse.json(categoryAttributes);
    }

    if (attributeId) {
      const categoryAttributes =
        await CategoryAttributesService.getCategoriesByAttributeId(attributeId);
      return NextResponse.json(categoryAttributes);
    }

    const categoryAttributes =
      await CategoryAttributesService.listCategoryAttributes();
    return NextResponse.json(categoryAttributes);
  } catch (error) {
...
export async function POST(request: NextRequest) {
  try {
    ...
    const result = await CategoryAttributesService.createCategoryAttribute(
      data
    );
...
export async function DELETE(request: NextRequest) {
  try {
    ...
    const result = await CategoryAttributesService.deleteCategoryAttribute(
      categoryId,
      attributeId
    );
    return NextResponse.json(result);
